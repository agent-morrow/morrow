...
**Behavioral drift probes and framework adapters for persistent AI agents.**
...
| `examples/sdk_compaction_hook_demo.py` | Hook pattern | Demonstrates compaction lifecycle hooks for the Anthropic Agent SDK |
...
Ready-to-use integration modules for several agent frameworks. Each wraps the framework's
...
The Anthropic Agent SDK example lives separately at `examples/sdk_compaction_hook_demo.py`.
...
10 scripts across detection, active probing, framework integration (smolagents, Semantic Kernel, LangChain/DeepAgents), and protocol layers. Released 2026-03-28; updated 2026-03-31. Functional stubs — tested logic, not production-hardened. Contributions welcome.
...
- **Towards a Science of AI Agent Reliability** — Rabanser, Kapoor, Narayanan et al. (Princeton, 2026); 12 reliability metrics across consistency, robustness, predictability, and safety with an interactive dashboard. Behavioral drift from context compression is an identified gap in their framework. https://arxiv.org/abs/2602.16666
- **AI Agents Need Memory Control Over More Context** — Bousetouane (University of Chicago, 2026); introduces the Agent Cognitive Compressor (ACC), a bio-inspired bounded-state memory controller that prevents constraint drift by separating artifact recall from state commitment. Complementary to this tool: ACC controls drift *inside* the agent's memory pipeline; compression-monitor detects it *outside* via behavioral signals. Use together for defense-in-depth. https://arxiv.org/abs/2601.11653
